...
godk_program %>% 
    filter(kursnamn == "Economics A") %>% 
    ggplot(aes(akademiskt_ar, andel, color = programstudent)) + 
    geom_line() + 
    labs(title = "Andel godkända studenter A kurs")
```
...
kursresult %>% 
    ...
    ggplot(aes(duration)) + 
        ...
        labs(title = "Studietid A kurs", x = "År", y = "Andel" ) + 
...
icke_godkända = kursresult %>% 
    filter(kursnamn == "Economics A", betyg == "U", akademiskt_ar == 2015 | akademiskt_ar == 2016 ) %>% 
    select(pnr, kursid)
```
...
Drygt 20 percent har bara en återstående kurs.

```{r}
utan_betyg = tibble( delkursantal = 0, n = icke_godkända %>% 
...
med_betyg = godkprov %>% 
    ...
    group_by(delkursantal) %>% 
    tally() 

bind_rows(utan_betyg, med_betyg) %>% 
    mutate(
        andel = round(n / sum(n) * 100, 1)
    ) %>% 
    kable()
```
