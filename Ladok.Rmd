...
Antalet studenter per akademiskt år är relativt konstant över tiden.
...
kursresult %>% 
    ...
    summarise(antal = n()) %>% 
    ggplot(aes(akademiskt_ar, antal)) + geom_line() + labs(title = "Registerade studenter")
...
```{r}
kursresult %>% 
    filter(betyg != "U", kursnamn == "Economics A", duration > 0) %>% 
    # group_by(kursnamn ) %>% 
...
andelar %>% 
    ...
    ggplot(aes(termindatum, andel, color = betyg )) + 
        geom_line() + 
        labs(title = "Andel godkänd A kurs", x ="Starttermin")
...
Totalt `r godkprov %>% filter(kursnamn != "Economics A") %>% tally() %>% as.numeric()` godkända studenter. Antal godkända per kurs sedan januari 2016.

```{r}
library(knitr)
library(stringr)
godkprov %>% 
    filter(kursnamn != "Economics A", year(idatum) >= 2016) %>% 
...
godkprov %>% 
    ...
    mutate(veckor = duration / 7) %>% 
    ggplot(aes(veckor)) + geom_histogram(bins = 50) + labs(title = "Antal veckor för uppsats")
```
